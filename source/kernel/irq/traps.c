...
void PF_handler_copy_on_write(ph_addr_t PF_vm)
{
    ...
    if(!phm)
    {
        dbg_error("访问非法内存，原因可能是父进程已经释放，依旧想访问父进程的全局变量\r\n");
        hlt();
    }
    ph_addr_t dest_vm = PF_vm;
...
void do_handler_page_fault(exception_frame_t *frame)
{
    ...
    pte_t *PF_pte = mmu_from_vm_get_pte((pde_t *)task_get_page_dir(cur_task), PF_vm);
    // 如果是写时复制，故意制造的缺页异常，就不报错里了
    if (PF_pte->write_disable == 0 && PF_pte->ignore == PTE_IGNORE_COPY_ON_WRITE &&frame->error_code & ERR_PAGE_WR)
    {
