...
int tty_open (device_t * dev)  {
	int idx = dev->minor;
	if ((idx < 0) || (idx >= TTY_NR)) {
		dbg_info("open tty failed. incorrect tty num = %d", idx);
		return -1;
	}
    
	tty_t * tty = tty_devs + idx;
	tty_fifo_init(&tty->ofifo, tty->obuf, TTY_OBUF_SIZE);
	sys_sem_init(&tty->osem, TTY_OBUF_SIZE);
	tty_fifo_init(&tty->ififo, tty->ibuf, TTY_IBUF_SIZE);
	sys_sem_init(&tty->isem, 0);
...
void tty_in (char ch) {
	...
	// 辅助队列要有空闲空间可代写入
	if (sys_sem_count(&tty->isem) >= TTY_IBUF_SIZE) {
		return;
	}

	// 写入辅助队列，通知数据到达
	tty_fifo_put(&tty->ififo, ch);
	sys_sem_notify(&tty->isem);
}
